...
class RulesView extends View{constructor(){super();const a=View.createWhiteButton(this.baseElement,{imageId:Icon.Clear,text:Strings.Close},this.back.bind(this),"small");a.style.position="absolute";a.style.left=extraLargeMarginCss;a.style.top=extraLargeMarginCss}async attach(){}async detach(){}destroyInternal(a){}back(a){this.fadeToPrevious();return!0}}
...
function setup(){function a(){View.loading=!1;bgMusic&&(bgMusic.onplay=function(){musicPlaying=!0;updateMusicButton()},bgMusic.onpause=function(){musicPlaying=!1;updateMusicButton()});View.createInitialView()}Strings.init();View.loading=!0;"serviceWorker"in navigator&&window.addEventListener("beforeinstallprompt",beforeInstallPrompt);window.onpopstate=View.windowHistoryStatePopped;window.onresize=adjustWindowSize;adjustWindowSize();FullscreenControl.onfullscreenchange=fullscreenChanged;Promise.all([async function(){try{if("AudioContext"in
window&&"AudioBuffer"in window&&"AudioBufferSourceNode"in window&&"fetch"in window){audioContext=new AudioContext;audioContext.suspend();audioBuffer=await audioContext.decodeAudioData(await (await fetch("assets/sounds/bg.mp3")).arrayBuffer());return}}catch(b){audioBuffer=audioContext=null}bgMusic=document.createElement("audio");bgMusic.loop=!0;bgMusic.src="assets/sounds/bg.mp3";document.body.appendChild(bgMusic);bgMusic.load()}(),Alien.preloadAllImages()]).then(a,a)}window.spacetrainStepMAIN=!0;
window.spacetrainCheckSetup&&window.spacetrainCheckSetup();
